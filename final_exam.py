...
st.set_page_config(page_title='Bike store sales Dashboard', page_icon='https://vmn-bike-eu.imgix.net/2023/10/bike-europe-peugeot-cycles-new-ebikes.jpg?auto=compress%2Cformat&q=50')
...
with Home:
    ...
    st.subheader('Click the button to see the dataset')
...
def main():
    ...
    # Assuming df is your DataFrame containing the 'Customer_Age' column
    # You can replace this with your data loading logic using file uploader or another method
    # Replace 'your_file.csv' with your file name
    
    # Display mean
    mean_age = df['Customer_Age'].mean().round(2)
    st.markdown(f"The mean age of customers is: {mean_age}")
...
def main():
    ...
    st.write("### Mean Order Quantity for Orders with Revenue > $10,000:")
    st.write(mean_order_quantity_greater_10k)

    st.write("### Mean Order Quantity for Orders with Revenue < $10,000:")
    st.write(mean_order_quantity_less_10k)
...
def main():
    ...
    st.write("### Orders Made in May 2016:")
    st.write(orders_may_2016)

    st.write("### Orders Made between May and July 2016:")
    st.write(orders_may_to_july_2016)
...
def main():
    ...
    st.write(df)  # Displaying original data

    st.write("### Applying 7.2% Tax on Sales within the United States:")
    modified_data = apply_tax_on_sales(df.copy())  # Applying tax on sales
